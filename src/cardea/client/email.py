"""
Email client for Cardea — list, read, delete, send, and reply to emails.
...
- ``DELETE /messages/{message_id}`` — permanently delete a message by UID
...
``delete_message`` returns: ``{deleted: true}``
...
def delete_message(
    ...
    """Permanently delete an email message by IMAP UID.
    ...
    Returns:
        Dict with ``deleted`` key set to ``True`` on success.
    """
    url = _resolve_base_url(base_url)
