...
from cardea.proxies._proxy_utils import streaming_proxy, strip_headers
from cardea.secrets import get_secret

logger = logging.getLogger(__name__)


# The generic proxy also strips "authorization" from forwarded headers because
# it re-injects its own credentials via _inject_auth().
_EXTRA_HOP_BY_HOP: frozenset[str] = frozenset(["authorization"])
...
def _make_handler(service_name: str, upstream: str, auth: dict[str, Any]) -> Any:  # noqa: ANN401
    ...
    async def _handler(path: str, request: Request) -> StreamingResponse:
        ...
        headers = strip_headers(request, extra_hop_by_hop=_EXTRA_HOP_BY_HOP)
        ...
        return await streaming_proxy(
            request, upstream_url, headers, extra_hop_by_hop=_EXTRA_HOP_BY_HOP
        )
